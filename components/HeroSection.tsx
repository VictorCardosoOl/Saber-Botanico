  ...
  <div className="flex flex-col gap-10 flex-1 lg:max-w-[50%] z-10 animate-fade-in-up">
    ...
    <div className="mt-12 pt-8 border-t border-white/5">
      ...
      <div className="flex items-center gap-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
        <span className="font-serif text-xl italic text-white" title="Vogue Living">Vogue Living</span>
        <span className="font-serif text-xl font-bold text-white" title="Architectural Digest">AD</span>
        <span className="font-sans text-lg font-bold tracking-widest text-white" title="Elle Decor">ELLE DECOR</span>
        <span className="font-serif text-xl text-white" title="Monocle">Monocle</span>
      </div>
    </div>
  </div>
);
  ...
  <div className="relative w-full flex-1 lg:h-auto flex justify-center lg:justify-end group perspective-1000">
    ...
    <div className="relative z-10 w-full overflow-hidden rounded-t-[10rem] rounded-b-xl shadow-2xl shadow-black/80 aspect-[4/5] border border-white/5 bg-forest-dark">
      <div 
        ...
        role="img"
        aria-label="Monstera Albo Variegata em destaque"
      >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 glass-panel border-t border-white/10">
        <div className="flex items-end justify-between">
          ...
          <button 
            className="size-12 rounded-full border border-gold/50 text-gold hover:bg-gold hover:text-forest-dark transition-all flex items-center justify-center"
            aria-label="Ver detalhes da Monstera Albo"
          >
...
const FeatureItem: React.FC<{ icon: string; title: string; desc: string }> = ({ icon, title, desc }) => (
  <div className="flex flex-col gap-3 p-4 hover:bg-white/5 transition-colors rounded-sm cursor-default group">
    <span className="material-symbols-outlined text-gold text-2xl font-light group-hover:scale-110 transition-transform origin-left">{icon}</span>
    <div>
      <h3 className="font-sans text-gold-light text-xs font-bold uppercase tracking-wider mb-1">{title}</h3>
      <p className="text-[10px] text-sage font-light leading-relaxed">{desc}</p>
    </div>
  </div>
);
